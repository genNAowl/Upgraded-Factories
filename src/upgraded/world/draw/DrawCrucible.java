...
public class DrawCrucible extends DrawBlock{
    ...
    public void draw(GenericCrafterBuild entity){
        ...
        if(warmup > 0f && flameColor.a > 0.001f){
            ...
            for(int i = 0;i < 4;i ++){
                Drawf.tri(x, y, 6f, 5 + Mathf.absin(Time.time, 25f, 10f), i*90 + Mathf.absin(Time.time, 50f, 360f));
            }
            ...
            for(int i = 0;i < 4;i ++){
                Drawf.tri(x, y, 6f, Mathf.absin(Time.time, 25f, 10f), i*90 + Mathf.absin(Time.time, 50f, 360f));
            }
            Draw.color();
        }
    }
